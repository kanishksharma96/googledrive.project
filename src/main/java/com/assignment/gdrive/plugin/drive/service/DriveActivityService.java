...
public class DriveActivityService {
    ...
    public List<DriveActivity> queryOnDrive(QueryDriveActivityRequest queryRequest) throws IOException {
        ...
        QueryDriveActivityResponse result =
                service.activity().query(queryRequest).execute();
            ...
            for (DriveActivity activity : activities) {
                ...
                List<String> actors =
                        activity.getActors().stream()
                                .map(DriveActivityService::getActorInfo)
                                .collect(Collectors.toList());
                List<String> targets =
                        activity.getTargets().stream()
                                .map(DriveActivityService::getTargetInfo)
                                .collect(Collectors.toList());
                System.out.printf(
    ...
    public static Credential authorize() throws IOException {
        // Load client secrets.
        InputStream in = DriveActivityService.class.getResourceAsStream(CREDENTIALS_FILE_PATH);
        if (in == null) {
        ...
        Credential credential =
                new AuthorizationCodeInstalledApp(flow, new LocalServerReceiver.Builder().setPort(62301).build())
                        .authorize("user");
